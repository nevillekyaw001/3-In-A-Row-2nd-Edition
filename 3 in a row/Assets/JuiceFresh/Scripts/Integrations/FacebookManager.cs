...
public class FacebookManager : MonoBehaviour
{
    ...
    {
        THIS = this;
        //        FB.Mobile.ShareDialogMode = ShareDialogMode.FEED;//1.4.10
    }
    ...
    IEnumerator loadPicture(string url)//1.4.6
    {
        WWW www = new WWW(url);
        yield return www;
    ...
    {
        FB.API("/" + id + "/picture?g&width=128&height=128&redirect=false", HttpMethod.GET, this.ProfilePhotoCallback);//1.4.6
    }
    ...
    {
        FB.API("me/friends?fields=picture.width(128).height(128)", HttpMethod.GET, RequestFriendsCallback);//1.4.9
    }
    ...
    private void RequestFriendsCallback(IGraphResult result)
    {
        if (!string.IsNullOrEmpty(result.RawResult))
        {
            ...
            {
                var dataArray = (List<object>)resultDictionary["data"];//1.4.6
    ...
    IEnumerator GetPictureCor(string url, FriendData friend)
    {
        ...
        WWW www = new WWW(url);
        yield return www;
        var sprite = Sprite.Create(www.texture, new Rect(0, 0, 128, 128), new Vector2(0, 0), 1f);//1.4.9
