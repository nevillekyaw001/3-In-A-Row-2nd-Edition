...
public class PlayFabDataManager : IDataManager {
	...
	bool setScoreRequested;

	public  void SetPlayerScore (int level, int score, Action callback)
	{
		if (setScoreRequested) return;
		setScoreRequested = true;
		UpdatePlayerScoreFoLeadboard (score);
		Debug.Log("set player score");
...
//					Debug.Log ("    " + item.StatisticName + " == " + item.Value);
	...
	void UpdatePlayerScoreFoLeadboard (int score) {
		...
		foreach (var playerData in NetworkManager.leadboardList)
		{
	...
	bool setLevelRequested;
	public  void SetPlayerLevel (int level)
	{
		if (setLevelRequested) return;
		setLevelRequested = true;
		Debug.Log("set player level");
