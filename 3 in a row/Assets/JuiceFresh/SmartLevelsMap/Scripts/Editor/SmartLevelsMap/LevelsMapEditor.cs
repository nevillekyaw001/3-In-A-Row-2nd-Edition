...
public class LevelsMapEditor : LevelsEditorBase {
    ...
    private void DrawLevelsSettings () {
        ...
        {
            Transform transform = _levelsMap.transform.Find("Path");
			if(transform)
            	_levelsMap.WaypointsMover.Path = transform.GetComponent<Assets.Plugins.SmartLevelsMap.Scripts.Path>();
        }
	...
	private void GenPath(List<MapLevel> levels)
	{
		GeneratePath(levels);
		_levelsMap.WaypointsMover = FindObjectOfType<WaypointsMover>();
		_levelsMap.WaypointsMover.transform.position = levels[0].transform.position;
	}
